...
class evoics_cron{
	...
	function schedule_jobs(){
		...
		if( $timestamp == false ) {
			...
			if( !empty($this->options['evoics_import_type']) 
				...
				$cron_data = $evo_cron->get_cron_data($cron_hook);
	...
	function fetching_action(){
		...
		if(!empty($this->options['evoics_file_url']) && $this->options['evoics_import_type'] !='manual_file' && $this->options['evoics_import_type'] !='manual_link'){
			...
			$events = $fnc->get_events_from_ics($this->options['evoics_file_url']);

			$log = array();
			$log['time']= time();

			if(!empty($events) && sizeof($events)>0){
				$process_events = $fnc->process_fetched_events($events);

				$good = $bad = 0;
				foreach($process_events as $index=>$event){
					$status = $fnc->import_event($event);
			...
			// record cron log
			$evo_cron = new evo_cron();
			$evo_cron->record_log($log,'evoics');
		}
