...
class EVOICS_ajax{
	...
	// run manual cron
		function evoics_002(){
			...
			$cron_data = $evo_cron->get_cron_data('evoics_schedule_action');
			$cron_result = $evo_cron->run_cron('evoics_schedule_action', $cron_data['sig']);
		...
		public function evoics_001(){
			...
			if(!isset($_POST['events'])){
				...
				foreach($event_list as $eventRow){
					...
					$event_id = $evoics_fnc->import_event($processedDATA);
