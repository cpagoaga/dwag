...
class EVOICS_settings{
					...
					$cutomization_pg_array = array(
						array(
							...
							'fields'=>array(
								...
								array(
									'id'=>'EVOICS_auto_timezone',
									'type'=>'yesno',
									'name'=>'Enable event time  adjusting based on this website\'s timezone'
								),array(
					...
					switch ($this->options['evoics_import_type']){
						...
						case 'schedule_monthly':
							...
							<?php endif;?>
					 		...
					 		?>
					 		<p><b><?php _e('Next Run','eventon');?>:</b> <?php echo $next_run_time . ' ('. $fnc->time_since( time(), $next_run) . ')';?></p>
					 		<?php endif;?>
						...
						default:
							...
							?>
							<input type="hidden" name='file_type' value='file_link'>
							<input type='file' name='events_ics_file'/>
	...
	// display fetched events list
		function display_events(){
			...
			// get the file URL
				if( !empty($this->options['evoics_import_type']) && $this->options['evoics_import_type']=='manual_link'){
				...
				}elseif(!empty($this->options['evoics_import_type']) && $this->options['evoics_import_type']=='manual_file'){
			...
			foreach($process_events as $index=>$event){
				...
				$ajde->wp_admin->table_row(
					array(						
						...
						'description'=> "<span class='".(!empty($event['DESCRIPTION'])?'check':'bar')."'></span>",
						'start_date_time'=> $event['event_start_date']."<br/>".$startTime,
						'end_date_time'=>$event['event_end_date']."<br/>".$endTime,
			...
			function hidden_fields($ics_data, $count){	
				...
				foreach($this->fnc->get_all_fields() as $field){
					...
					if(in_array($field, $textarea_fields)){
						$output .= "<textarea class='evoics_event_data_row' style='display:none' name='events[{$count}][{$field}]'>". (!empty($ics_data[$field])? addslashes($ics_data[$field]):'')."</textarea>";
					}else{
