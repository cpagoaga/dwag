...
if( ! function_exists("vctm_load_templates") ){
	function vctm_load_templates($folder = 'default_templates', $hook_prefix = 'vctm')
	{
		...
		foreach (glob($folder) as $filename)
		{
			...
			$data['image_path'] = preg_replace( '/\s/', '%20', plugins_url( 'assets/img/custom_template_thumbnail.jpg', __FILE__ ) ); // Always use preg replace to be sure that "space" will not break logic. Thumbnail should have this dimensions: 114x154px
