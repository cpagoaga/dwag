...
function stag_required_plugins() {
	$plugins = array(
		array(
			'name'             => __( 'WooCommerce - excelling eCommerce', 'stag' ),
			'slug'             => 'woocommerce',
			'required'         => true,
			'force_activation' => true
		),
		array(
			'name'             => __( 'StagTools', 'stag' ),
			'slug'             => 'stagtools',
			'required'         => true,
			'force_activation' => true
		),
		array(
			'name'             => __( 'Stag Custom Sidebars', 'stag' ),
			'slug'             => 'stag-custom-sidebars',
			'required'         => true,
		),
		array(
			'name'             => __( 'LayerSlider WP', 'stag' ),
			'slug'             => 'LayerSlider',
			'source'           => get_stylesheet_directory() . '/config-layerslider/layerslider.zip',
			'required'         => false,
			'version'          => '5.2.0'
		)
	);
...
function stag_display_static_content() {
	...
	if ( is_page() && stag_get_option('static_page_visible_on_single-pages') === "on" ) $is_visible = true;

	if ( is_single() && 'post' == get_post_type() && stag_get_option('static_page_visible_on_single-posts') === "on" ) $is_visible = true;

	if ( is_archive() && 'post' == get_post_type() && stag_get_option('static_page_visible_on_archive-pages') === "on" ) $is_visible = true;

	if ( is_archive() && 'product' == get_post_type() && stag_get_option('static_page_visible_on_shop-archive-pages') === "on" ) $is_visible = true;

	if ( is_single() && 'product' == get_post_type() && stag_get_option('static_page_visible_on_single-product-pages') === "on"  ) $is_visible = true;

	if ( is_404() && stag_get_option('static_page_visible_on_404-page') === "on" ) $is_visible = true;

	if ( is_home() || is_front_page() ) {
		if ( stag_get_option('static_page_visible_on_homepage') === "on" ) {
			$is_visible = true;
		} else {
			$is_visible = false;
		}
	...
	if ( $wp_query->is_posts_page ) {
		$is_visible = false;
		if ( stag_get_option('static_page_visible_on_blog-page') === "on" ) $is_visible = true;
	}
